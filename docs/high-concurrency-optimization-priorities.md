...
上面的判断主要基于前 10 轮结果。但在第 11 到第 21 轮连续优化和复测之后，当前系统状态已经发生了明显变化。

因此需要补一个阶段性修正：

1. 读链路和订单主链路前半段，已经不再是当前首要矛盾
2. 幂等路径和商品 / SKU 读取批量化，已经证明是正确方向
3. 当前系统已经具备一版可用的“稳定基线”
4. 后续优化重点，应该从“先把链路打通”切换到“在稳定基线上继续压低时延和提高容量”

### 8.1 已经证明效果不错的优化项

以下几类优化，已经通过多轮压测证明是有效的，不应轻易回退。
...
1. 幂等记录从“先查再写”收缩成“先尝试创建 pending，冲突再回查”
2. 状态回写优先按 `id` 更新，而不是反复按 `(action_type, action_key)` 查找

这类优化的直接效果是：

1. `order_action_records` 不再像早期那样成为明显瓶颈
2. 幂等路径不再是后续轮次的首要失败来源
...
1. 订单项已经改成批量插入
2. 去掉了一部分事务后的额外回库
3. `order-service` 连接池上调后，普通 SKU 场景有明显改善

这类优化的直接效果是：

1. 普通 SKU 场景在高压下的时延明显低于未优化前
2. `normal-v80` 等样本已经体现出订单侧优化的收益
...
1. 不是所有库存优化都已经做得很好
2. 但当前这版“稳定优先”的库存实现，已经被证明可以作为稳定基线

当前有效的库存策略是：

1. `Reserve` 采用“先更新库存，再写 reservation”
2. 避免在热点事务里引入更激进的流程
3. 热点 SKU 本地限并发采用更保守但不过紧的阈值

从后续轮次结果看：

1. 激进版热点库存实现会把热点场景直接打崩
2. 回到稳定优先实现后，`hot-v20 / hot-v40 / hot-v80` 都恢复成了 `0` 业务失败

这说明当前库存侧已经有一版“可用、稳定、经压测验证”的基线实现。

### 8.2 当前还没有彻底做好的部分

虽然很多关键问题已经被解决，但还不能认为系统已经完全优化完成。

当前仍值得继续推进的部分主要有两类。

#### 热点库存场景下的进一步时延优化

当前热点链路已经稳定，但时延还没有完全压回到最优状态。

因此库存侧接下来的目标，不再是“先救成功率”，而是：

1. 在当前稳定实现上继续优化热点场景时延
2. 避免再次引入第十九轮那种激进实现导致的回归

#### 订单服务数据库 / 持久化的深化优化

虽然订单侧已经有明显收益，但这块还不是终点。
...
也就是说，订单侧后续更适合做“细化优化”，而不是再做大幅度、方向性的重构。

### 8.3 现阶段的优先级应如何调整

结合第 11 到第 21 轮的结果，当前更合理的优先级应该调整为：
...
1. `order-service` 的 DB / 持久化路径
2. 热点库存场景下的时延进一步下降
3. 更完整的普通 SKU、热点 SKU、checkout 对照验证
...
1. 当前系统的主要收益，仍然来自交易主链路和热点库存的工程化收缩
2. 更重的架构演进成本高，但当前压测并没有证明必须立刻走到那一步

### 8.4 当前的实际建议

如果基于当前最新状态只保留一句话建议，那就是：

先把已经证明有效的优化项固化下来，把它们视为当前稳定基线；后续优化重点放在订单服务 DB / 持久化的进一步收缩，以及热点库存场景下的时延继续下降，而不是重新引入更激进的热点库存实现或过早做全面架构升级。
