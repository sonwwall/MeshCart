...
- Consul 注销与实例摘流顺序
- 发布、重启、异常退出时的最小运维约定
- 远程 MySQL / Redis 场景下的连接、超时、认证与排障约定
...
- 当前主开发模式是宿主机启动服务，如果不先把启动契约和排障约定统一，后续每加一个服务都会再散一次

当前阶段建议先做到：

- 保持业务服务继续以宿主机脚本启动为默认调试方式
- MySQL / Redis 继续使用远程环境，不把本地 Compose 化作为当前前置条件
- 对外暴露可用于存活与就绪判断的检查入口
- 服务收到退出信号后，先停止接收新流量，再等待在途请求结束
- 使用 Consul 时，明确“先摘流再停进程”还是“先停进程再注销”的统一约定，并写入 runbook
- 让 `gateway` 和下游 RPC 服务都遵循同一套关闭流程，避免不同服务各写一套
- 把远程依赖的连通性检查、启动前自检和故障定位步骤写入 runbook
...
- 先补最小护栏，再加业务，性价比最高
- 当前最小治理基线应优先围绕“宿主机调试友好”展开，而不是先切换部署形态
- 等项目基本完成后，再统一补业务服务容器化和 Compose 交付形态更合适
...
1. 补服务生命周期治理
2. 补远程 MySQL / Redis 场景下的启动、配置、排障闭环
3. 补测试与最小 CI 门槛
4. 继续推进下一个核心业务模块
5. 根据真实调用链问题，再决定是否补重试、熔断、隔离
6. 项目基本完成后，再推进业务服务容器化与 Compose 化
...
- 当前项目还在服务治理基线收口阶段，过早推进 `order/payment` 这类高事务复杂度链路，收益不高
- 更合适的是先把现有服务的启动、注册、退出、排障、测试基线收敛
- 再优先选择 `cart` 或 `inventory` 这类更容易复用现有链路的业务服务
- 订单与支付更适合放到治理能力和服务模板更稳定之后推进

换句话说，当前阶段最合理的路线不是“先做完整治理”，而是“先做治理底线，再用下一阶段业务继续验证治理需求”。
