...
在第十九轮之前，先对 `order-service` 做了一版验证型优化，目标是压低第十七、十八轮里已经开始抬升的订单侧 DB 压力：

1. `TransitionStatus` 去掉事务后的额外回库，直接返回事务内已更新的订单对象
2. `cancel/pay_confirm` 复用已有订单项，避免状态流转后再次回读订单明细
3. `order-service` 连接池配置上调：
   `max_open_conns: 60 -> 120`
   `max_idle_conns: 20 -> 40`

第十九轮不是继续盲目升压，而是验证这版订单侧优化是否真的让时延平台和 DB wait 回落。
...
- `loadtest/results/round19-order-hot-v40.json`
- `loadtest/results/round19-order-hot-v80.json`
- `loadtest/results/round19-order-normal-v80.json`

说明：

1. 第十九轮前先重启了 `order-service`，确保最新订单侧优化和连接池配置已生效
2. `gateway` 和 `inventory-service` 继续沿用前几轮的干净配置
...
- 成功请求数：`75`
- 业务失败率：`87.88%`
- `order_create_duration avg`：`1948.85ms`
- `order_create_duration p95`：`2032.31ms`
...
- 成功请求数：`155`
- 业务失败率：`87.83%`
- `order_create_duration avg`：`1925.03ms`
- `order_create_duration p95`：`2010.40ms`
...
- 成功请求数：`7503`
- 业务失败率：`0`
- `order_create_duration avg`：`269.37ms`
- `order_create_duration p95`：`497.91ms`

### 32.4 第十八轮 / 第十七轮 vs 第十九轮对比

第十九轮最重要的变化，不是订单侧优化“部分生效”，而是热点 SKU 场景发生了明显回归。

#### 热门 SKU 池

相对第十六轮 `hot-v40`：

- 吞吐：`115.45 req/s -> 20.22 req/s`
- 成功请求数：`3514 -> 75`
- 业务失败率：`0 -> 87.88%`
- `avg`：`292.57ms -> 1948.85ms`
- `p95`：`397.38ms -> 2032.31ms`

相对第十七轮 `hot-v80`：

- 吞吐：`94.60 req/s -> 40.77 req/s`
- 成功请求数：`2925 -> 155`
- 业务失败率：`0 -> 87.83%`
- `avg`：`782.15ms -> 1925.03ms`
- `p95`：`890.45ms -> 2010.40ms`

这说明：

1. 第十九轮后，热点 SKU 路径重新出现了严重退化
2. 问题不是轻微回退，而是已经回到高失败率、高长尾的失稳状态
3. 当前这版订单侧优化并没有带来热点场景下的正向收益
...
- 吞吐：`144.36 req/s -> 247.31 req/s`
- 业务失败率：`0 -> 0`
- `avg`：`492.84ms -> 269.37ms`
- `p95`：`762.10ms -> 497.91ms`

这说明：

1. `order-service` 连接池和持久化路径优化，对普通 SKU 场景是有正向效果的
2. 第十九轮的严重问题，不是订单服务整体崩了，而是热点链路重新暴露出了库存预占问题
...
`order-service` 当前指标显示：

- `meshcart_db_wait_count_total{service="order-service"}`：`0`
- `meshcart_biz_errors_total{service="order-service", module="order", action="create", stage="reserve_rpc", code="1009999"}`：`1639`

这说明：

1. 订单侧数据库等待在本轮并没有继续放大
2. 失败主要不是来自 `persist`，而是大量集中在 `reserve_rpc`

#### `inventory-service`

`inventory-service` 当前指标显示：

- `meshcart_inventory_reservation_requests_total{action="reserve", outcome="success", reason="ok"}`：`27723`
- `meshcart_db_wait_count_total{service="inventory-service"}`：`0`

但日志里大量出现：

1. `inventory reserve stock update failed: context deadline exceeded`
2. `inventory reserve repository failed`
3. 热点 `sku_id=2036361111032307713` 的预占超时和回库超时

这说明第十九轮里：

1. 热点库存预占重新成为首个显式失败点
2. 问题主要体现为热点 `sku` 下的大量超时，而不是订单侧 DB wait
3. 当前版本下，“订单侧优化后让热点库存瓶颈重新暴露”这个判断比“订单侧优化失败”更准确

### 32.6 第十九轮结论

第十九轮最重要的结论：

1. 订单侧优化对普通 SKU 场景是有效的
2. 但热点 SKU 场景发生了严重回归，失败率重新升到 `87%+`
3. 当前首瓶颈已经重新暴露回热点库存预占，而不是订单服务 DB wait
4. 下一步不能继续沿着订单侧优化往前推，应该先回到热点库存预占治理

### 32.7 下一步建议

基于第十九轮结果，建议下一步按这个顺序推进：

1. 先回查 `inventory-service` 当前热点预占实现，重点看热点 `sku` 下为什么大量 `context deadline exceeded`
2. 检查 `Reserve` 里最近的实现调整是否放大了热点冲突，例如：
   先写 reservation 再更新 stock
   事务内最终回查库存
   更严格并发阈值与当前事务路径之间的相互作用
3. 优先恢复一个“热点场景更稳”的库存预占实现，再做热点验证
4. 不要把第十九轮结果解读为订单侧优化整体无效；它对普通 SKU 已经体现出收益

### 32.8 本轮停止原因

第十九轮没有继续扩更多验证样本，原因是：

1. 热点 SKU 场景已经出现明显回归，且失败率高到足以说明问题
2. 当前最需要做的是回到库存热点治理，而不是继续扩压测覆盖
3. 这轮已经足够证明：订单侧优化后，新的主要故障重新暴露在热点库存预占路径

## 33. 第二十轮结果

### 33.1 第二十轮前已落地优化

在第二十轮之前，先把热点库存预占路径回调到“稳定优先”的实现：

1. `Reserve` 恢复为先更新 `inventory_stocks`，成功后再写 `inventory_reservations`
2. 去掉热点事务里的更激进流程，恢复预占成功后的事务外库存查询
3. 热点 SKU 本地限并发阈值从 `2` 放宽到 `4`

第二十轮的目标不是继续找上限，而是验证第十九轮的热点回归是否已经被压住。
...
- `loadtest/results/round20-order-hot-v20.json`
- `loadtest/results/round20-order-hot-v40.json`

说明：

1. 第二十轮前先重启了 `inventory-service`，确保热点库存回调已实际生效
2. `gateway` 和 `order-service` 继续沿用前几轮的干净配置
...
- 成功请求数：`714`
- 业务失败率：`0`
- `order_create_duration avg`：`798.12ms`
- `order_create_duration p95`：`1174.79ms`
...
- 成功请求数：`1444`
- 业务失败率：`0`
- `order_create_duration avg`：`792.73ms`
- `order_create_duration p95`：`1181.32ms`

### 33.4 第十九轮 vs 第二十轮对比

第二十轮最重要的结果，是第十九轮的热点回归已经被明显压住。

#### 热门 SKU 池

相对第十九轮 `hot-v40`：

- 吞吐：`20.22 req/s -> 46.49 req/s`
- 成功请求数：`75 -> 1444`
- 业务失败率：`87.88% -> 0`
- `avg`：`1948.85ms -> 792.73ms`
- `p95`：`2032.31ms -> 1181.32ms`

这说明：

1. 热点库存预占路径回调后，第十九轮的严重回归已经消失
2. 当前热点 `v20 / v40` 已重新恢复为有效成功样本
3. 第十九轮的问题确实主要来自那版更激进的热点库存实现
...
说明第二十轮里没有继续新增大量 `reserve_rpc` 失败。

#### `inventory-service`

当前指标显示：

- `meshcart_inventory_reservation_requests_total{action="reserve", outcome="success", reason="ok"}`：`2158`
- `meshcart_db_wait_count_total{service="inventory-service"}`：`0`

同时未观察到：

1. 新的显式 `reserve` 失败分桶增长
2. 像第十九轮那样明显的热点预占超时放大

这说明第二十轮里：

1. 热点库存预占重新回到可控状态
2. 这次回调并没有重新引入新的显式失败源

### 33.6 第二十轮结论

第二十轮最重要的结论：

1. 热点库存预占路径回调后，第十九轮的热点回归已经被压住
2. 当前热点 `v20 / v40` 已恢复成 `0` 业务失败
3. 这证明热点库存链路应优先采用“稳定优先”的事务顺序，而不是更激进的热点实现
4. 但当前热点时延仍处于偏高平台，尚未回到第十六轮那种更低水平

### 33.7 下一步建议

基于第二十轮结果，建议下一步按这个顺序推进：

1. 保留当前这版更稳的热点库存实现，不要再回到第十九轮那种更激进顺序
2. 补一轮更高热点验证，例如 `hot-v80`，确认当前稳定区间是否已经恢复
3. 必要时再补 `normal-v80` 对照，确认订单侧优化仍然保留收益
4. 在没有新证据前，不建议继续改热点库存事务顺序

### 33.8 本轮停止原因

第二十轮没有继续直接扩到更高热点压力，原因是：

1. 本轮核心目标是验证第十九轮热点回归是否已被压住
2. `hot-v20 / hot-v40` 已足够证明这次库存回调是有效的
3. 在结果落档之前，继续扩样本的优先级低于先固化本轮结论

## 34. 第二十一轮结果

### 34.1 第二十一轮目标

第二十一轮的目标，是在第二十轮已经恢复 `hot-v20 / hot-v40` 稳定后，继续确认更高热点压力下的稳定区间是否也已经恢复。

本轮主要回答一个问题：

1. 当前这版“稳定优先”的库存实现，能否把 `hot-v80` 也稳定住
...
- `loadtest/results/round21-order-hot-v80.json`

说明：

1. `gateway`、`order-service`、`inventory-service` 都沿用第二十轮之后的稳定版本
2. 本轮不再扩更多样本，而是专门确认热点高档位是否恢复
...
- 成功请求数：`4371`
- 业务失败率：`0`
- `order_create_duration avg`：`501.78ms`
- `order_create_duration p95`：`626.84ms`

### 34.4 第二十轮 / 第十九轮 vs 第二十一轮对比

第二十一轮最重要的结果，是热点高档位也已经重新恢复稳定。

#### 热门 SKU 池

相对第十九轮 `hot-v80`：

- 吞吐：`40.77 req/s -> 141.69 req/s`
- 成功请求数：`155 -> 4371`
- 业务失败率：`87.83% -> 0`
- `avg`：`1925.03ms -> 501.78ms`
- `p95`：`2010.40ms -> 626.84ms`

相对第二十轮 `hot-v40`：

- 吞吐：`46.49 req/s -> 141.69 req/s`
- 成功请求数：`1444 -> 4371`
- 业务失败率：`0 -> 0`
- `avg`：`792.73ms -> 501.78ms`
- `p95`：`1181.32ms -> 626.84ms`

这说明：

1. 第二十轮回调后的库存实现，不只是把热点低档位修好，热点高档位也已经恢复稳定
2. 当前热点场景已经明显优于第十九轮的回归状态
3. 热点时延平台也较第二十轮继续下降
...
说明本轮没有继续新增热点 `reserve_rpc` 失败。

#### `inventory-service`

当前指标显示：

- `meshcart_inventory_reservation_requests_total{action="reserve", outcome="success", reason="ok"}`：`6529`
- `meshcart_db_wait_count_total{service="inventory-service"}`：`0`
...
1. 热点库存预占路径在 `v80` 下仍然稳定
2. 当前版本下没有再次出现第十九轮那种热点超时放大

### 34.6 第二十一轮结论

第二十一轮最重要的结论：

1. 当前这版“稳定优先”的库存实现已经把 `hot-v80` 也稳定住了
2. 热点库存回归问题已经可以视为被彻底压住
3. 当前热点高档位已恢复为 `0` 业务失败样本
4. 这版库存实现可以作为当前稳定基线继续往后分析和优化

### 34.7 下一步建议

基于第二十一轮结果，建议下一步按这个顺序推进：

1. 把当前这版库存实现视为稳定基线，先不要再改热点库存事务顺序
2. 回到高并发优化优先级文档，重新整理当前阶段的主要优化方向
3. 如需继续压测，可再补 `normal-v80` 或 checkout 对照，确认订单侧优化收益是否仍然稳定
4. 后续新的优化重点，应建立在当前稳定基线上推进，而不是重新引入更激进的热点实现

### 34.8 本轮停止原因

第二十一轮没有继续直接扩更多热点样本，原因是：

1. 本轮已经足够证明 `hot-v80` 重新恢复稳定
2. 当前最重要的不是继续堆更多压测结果，而是把稳定基线和后续优化方向重新整理清楚
3. 在热点回归问题已经收敛后，继续扩样本的收益低于先更新文档与优先级判断
