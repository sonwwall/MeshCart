...
但业务服务仍主要依赖宿主机脚本启动，MySQL 也还未纳入统一本地依赖编排。
...
- `docker compose up` 可以拉起依赖和最小业务链路
- 宿主机手工启动模式继续保留，作为排障回退手段
- Consul 健康检查统一按容器网络重新梳理，减少 `127.0.0.1` / 容器网络混用问题

### 5.3 完成标准

- 新同学不需要手工拼装多套环境变量即可跑通主链路
- `runbook` 同步收敛到一套默认启动方式
- 最小链路启动时间和排障成本显著下降
