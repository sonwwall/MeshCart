...
func RunPreflight(cfg config.Config) error {
	...
	if strings.EqualFold(cfg.UserRPC.DiscoveryType, "consul") ||
		strings.EqualFold(cfg.CartRPC.DiscoveryType, "consul") ||
		strings.EqualFold(cfg.ProductRPC.DiscoveryType, "consul") ||
		strings.EqualFold(cfg.InventoryRPC.DiscoveryType, "consul") {
