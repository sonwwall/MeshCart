...
func GetOrder(svcCtx *svc.ServiceContext) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		logic := orderlogic.NewGetLogic(ctx, svcCtx)
		data, bizErr := logic.Get(identity.UserID, orderID)
		if bizErr != nil {
			logx.L(ctx).Warn("get order failed", zap.Int64("user_id", identity.UserID), zap.Int64("order_id", orderID), zap.Int32("code", bizErr.Code), zap.String("message", bizErr.Msg))
			c.JSON(200, common.Fail(bizErr, traceID))
			return
		}
