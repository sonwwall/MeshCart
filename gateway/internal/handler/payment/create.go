...
func CreatePayment(svcCtx *svc.ServiceContext) app.HandlerFunc {
	return func(ctx context.Context, c *app.RequestContext) {
		...
		if err := c.BindAndValidate(&req); err != nil {
			logx.L(ctx).Warn("create payment request bind failed", zap.Error(err))
		...
		logic := paymentlogic.NewCreateLogic(ctx, svcCtx)
		data, bizErr := logic.Create(identity.UserID, &req)
		if bizErr != nil {
			logx.L(ctx).Warn("create payment failed", zap.Int64("user_id", identity.UserID), zap.Int64("order_id", req.OrderID), zap.Int32("code", bizErr.Code), zap.String("message", bizErr.Msg))
