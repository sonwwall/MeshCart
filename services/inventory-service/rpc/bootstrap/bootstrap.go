...
import (
	...
	bizservice "meshcart/services/inventory-service/biz/service"
	"meshcart/services/inventory-service/config"
	"meshcart/services/inventory-service/dal/db"
	rpchandler "meshcart/services/inventory-service/rpc/handler"
)
...
func Run() {
	...
	logx.L(nil).Info("inventory-service starting")
	if err := lifecycle.RunUntilSignal(
		svr.Run,
		func(ctx context.Context) error {
			draining.Store(true)
			if err := lifecycle.WaitForDrainWindow(ctx, drainTimeout("INVENTORY_SERVICE_DRAIN_TIMEOUT_MS")); err != nil {
				return err
			}
			logx.L(nil).Info("inventory-service shutting down", zap.Duration("timeout", shutdownTimeout()))
			if adminServer != nil {
		...
		logx.L(nil).Error("inventory-service stopped with error", zap.Error(err))
	}
}
...
func initOpenTelemetry() func() {
	otel := otelprovider.NewOpenTelemetryProvider(
		otelprovider.WithServiceName("inventory-service"),
...
func runPreflight(cfg config.Config) {
	timeout := lifecycle.TimeoutFromMS(getEnvAsInt("INVENTORY_SERVICE_PREFLIGHT_TIMEOUT_MS", 1500), 1500*time.Millisecond)
	checks := []lifecycle.PreflightCheck{
		{Name: "mysql", Address: cfg.MySQL.Address},
	}
	if strings.EqualFold(getEnv("INVENTORY_SERVICE_REGISTRY", "consul"), "consul") {
	...
	if err := lifecycle.RunTCPPreflight(ctx, checks...); err != nil {
		logx.L(nil).Fatal("inventory-service preflight failed", zap.Error(err), zap.Duration("timeout", timeout))
	}
	logx.L(nil).Info("inventory-service preflight passed", zap.Duration("timeout", timeout))
}
...
func startAdminServer(sqlDB *sql.DB, draining *atomic.Bool, statsInterval time.Duration) *http.Server {
	metricsAddr := getEnv("INVENTORY_METRICS_ADDR", ":9095")
	stopCollectDBStats := metricsx.StartDBStatsCollector("inventory-service", sqlDB, statsInterval)
	srv := &http.Server{
		Addr: metricsAddr,
		Handler: lifecycle.NewHTTPMux("inventory-service", metricsx.PromHandler(), func(ctx context.Context) error {
	...
	go func() {
		...
		logx.L(nil).Info("inventory-service admin server starting", zap.String("addr", metricsAddr))
		if err := srv.ListenAndServe(); err != nil && !errors.Is(err, http.ErrServerClosed) {
			logx.L(nil).Error("inventory-service admin server stopped with error", zap.Error(err))
		}
	}()
	return srv
}

func newServer(svc *bizservice.InventoryService) server.Server {
	serviceName := getEnv("INVENTORY_RPC_SERVICE", "meshcart.inventory")
	serviceAddr, err := mustResolveTCPAddr(getEnv("INVENTORY_SERVICE_ADDR", "127.0.0.1:8891"))
	if err != nil {
		logx.L(nil).Fatal("resolve inventory-service addr failed", zap.Error(err))
	}
	...
	return inventoryservice.NewServer(rpchandler.NewInventoryServiceImpl(svc), opts...)
}
...
func buildConsulCheck(serviceName string, serviceAddr *net.TCPAddr) *consulapi.AgentServiceCheck {
	...
	if strings.EqualFold(getEnv("INVENTORY_SERVICE_CONSUL_TCP_CHECK", "false"), "true") {
...
func shutdownTimeout() time.Duration {
	return time.Duration(getEnvAsInt("INVENTORY_SERVICE_SHUTDOWN_TIMEOUT_MS", 5000)) * time.Millisecond
}
