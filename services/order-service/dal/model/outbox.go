...
type OrderOutbox struct {
	...
	EventKey    string     `gorm:"column:event_key;type:varchar(128);not null;default:'';index:idx_order_outbox_event_key"`
	...
	Status      string     `gorm:"column:status;type:varchar(16);not null;default:'pending';index:idx_order_outbox_status_retry,priority:1"`
	RetryCount  int32      `gorm:"column:retry_count;not null;default:0"`
	MaxRetries  int32      `gorm:"column:max_retries;not null;default:16"`
	NextRetryAt *time.Time `gorm:"column:next_retry_at;index:idx_order_outbox_status_retry,priority:2"`
