...
func Load() (Config, error) {
	source := strings.ToLower(getEnv("CART_SERVICE_CONFIG_SOURCE", "file"))
	if source == "apollo" {
		return loadFromApollo()
	}

	path := getEnv("CART_SERVICE_CONFIG", "services/cart-service/config/cart-service.local.yaml")

	v := viper.New()
	v.SetConfigFile(path)
	v.SetConfigType("yaml")
	v.SetEnvPrefix("CART_SERVICE")
	...
	v.SetDefault("mysql.database", "meshcart_cart")
	...
	v.SetDefault("migration.source", "file://services/cart-service/migrations")
	v.SetDefault("snowflake.node", 3)
