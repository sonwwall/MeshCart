...
func Run() {
	...
	if err := lifecycle.RunUntilSignal(
		...
		func(ctx context.Context) error {
			draining.Store(true)
			if err := lifecycle.WaitForDrainWindow(ctx, drainTimeout("PRODUCT_SERVICE_DRAIN_TIMEOUT_MS")); err != nil {
				return err
			}
			logx.L(nil).Info("product-service shutting down", zap.Duration("timeout", shutdownTimeout()))
			if adminServer != nil {
...
func runPreflight(cfg config.Config) {
	timeout := lifecycle.TimeoutFromMS(getEnvAsInt("PRODUCT_SERVICE_PREFLIGHT_TIMEOUT_MS", 1500), 1500*time.Millisecond)
	...
	if err := lifecycle.RunTCPPreflight(ctx, checks...); err != nil {
		logx.L(nil).Fatal("product-service preflight failed", zap.Error(err), zap.Duration("timeout", timeout))
	}
	logx.L(nil).Info("product-service preflight passed", zap.Duration("timeout", timeout))
}
...
func shutdownTimeout() time.Duration {
	return time.Duration(getEnvAsInt("PRODUCT_SERVICE_SHUTDOWN_TIMEOUT_MS", 5000)) * time.Millisecond
}
